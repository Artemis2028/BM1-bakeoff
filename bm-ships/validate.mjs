...
check('Reman Warbird remains pack id 53; Remus vendor is not the sole key', () => {
  const ship = catalog.getShip(53);
  assert.equal(ship.id, 53);
  assert.equal(ship.key, 'bm-ship:53');
  assert.equal(ship.name, 'Reman Warbird');
  assert.equal(ship.rosterState, 'active');
  assert.equal(ship.shipyardEligible, false);
  assert.equal(manifest.aliases?.['53'], undefined);
  const funded = {credits:1e9, standings:{romulan:100}};
  assert.equal(catalog.getPurchaseDecision(53, funded).reason, 'restricted-stock');
});
