...
pack. On **BM1-bakeoff**, the catalog is **not** wired into spawning, yards, or
markets — live traffic still uses the pre-pack 66-hull path until a later wire
slice. See `../docs/SHIP-ECONOMY-REVIEW.md` and `../docs/GUIDED-CONVERGENCE.md`.
...
Pack helpers encode an initial tier ladder (open 0, trusted 15, respected 30,
military 50, strategic 75). Excalibur and Concord require 100. All 172 active
records now have explicit standing requirements, so another host does not need
to invent tier thresholds. A future record with neither an explicit requirement
nor a configured tier still refuses the sale.
...
Pack metadata still tags Project X in Paso (49/347), Concord at the Free Swiss
Reserve Exchange, and the Reman Warbird at Remus. On bake-off those yard tags
are **not** live stock: Reman hull **53 / `bm-ship:53`** stays a durable unlock
plus later `meetPackPurchaseDecision`. Without a vendor context the pack still
refuses `restricted-stock`, so Remus station is not the sole key. Dominion
heavy stock stays in the core region.
...
`ships.json` is the authored source for hulls. Old IDs are in `aliases` here and
in `integration-rules.json` `hullAliases`. On bake-off, `data/starship_manifest.json`
is **not** regenerated from this pack (guided `sync-ship-roster.mjs` was not imported).
