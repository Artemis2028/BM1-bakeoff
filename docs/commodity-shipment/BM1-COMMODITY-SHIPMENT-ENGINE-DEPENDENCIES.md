...
| One post-move price for book and shop; stored price clamped; demand falls on a sell; no refill on trade or load | A second price table, a pre-move settle, a shop path on a different price, a paid sell at the demand floor, or `restockToCap` |
...
| `applyShopSell` / `removeCargoFromPods` | This docs PR does not edit them. A later trade must settle the shop on the same post-move price as the book. If `removeCargoFromPods` has already taken the tagged pod, the sale row pays 0 and is dropped. Fail if a missing pod pays, or if a full purchase discards another pod. |
...
| `marketBook` / `tickMarketBook` / `restockOnLoadForbidden` | **Read** stock and demand clamps. Fail if this package instant-refills, stores a second price, or retunes `PHASE8_MAGNITUDES` or `tickDrift`. A later buy/sell settles at the post-move price, clamps stored price, and lowers demand on a sell. |
...
| A buy snaps stock to cap, settles at the pre-move price, or writes `price` on the commodity entry | Gate 13. |
