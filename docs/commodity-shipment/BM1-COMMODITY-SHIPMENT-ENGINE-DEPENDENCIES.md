...
| `sales` empty unless this book wrote the `saleId`; sell-back needs the tagged tons aboard and consumes the `saleId` once | A tractored, captured, salvaged, or prize lot becoming sellable; a pay with the pod gone; a second pay of the same `saleId`; `contraband` cleared by spoof or a friendly dock |
| One post-move price per ton for book and shop; an N-ton buy lowers stock by N; stored price clamped per ton; demand falls by one per sold ton; no refill on trade or load | A second price table, one step for many tons, a pre-move settle, a shop path on a different price, a paid buy past the stock floor, a paid sell at the demand floor, or `restockToCap` |
| Same-world and mixed round trips net ≤ 0; price returns after a mixed trip; no free repair | Any per-trip gain, a mixed trip that leaves price stepped, stock snapped back by a two-world loop, or `freeRepair: true` |
...
| Finite market | `PHASE8_MAGNITUDES` clamps (`stockCap` 8, `demandCap` 8, `floor` 0, `listPrice` 6, `tickDrift` 0). Those integers stay Phase 8’s. This package does not retune them and does not copy a remastered curve. `applyShopBuy` / `applyShopSell` change stock by one and do not change `price` today. `dealResult` and `sanitizeMarket` floor a price at 1. They do not cap the live stored `market.price`. `tickMarketBook` returns `restock-forbidden` when `restockToCap` is set. `restockOnLoadForbidden` is `lastLoadRestocked !== true`. A later trade adds this package’s own price floor, price cap, step, and recovery value. It does not write `tickDrift`. |
...
| `applyShopBuy` / `applyShopSell` / `removeCargoFromPods` | This docs PR does not edit them. A later engine may change the shop settle to the shared post-move per-ton rule and nothing else. `PHASE8_MAGNITUDES` stays unchanged. The Phase 8 suite changes only for that settle. If `removeCargoFromPods` has already taken the tagged pod, the sale row pays 0 and is dropped. Fail if a missing pod pays, or if a full purchase discards another pod. |
...
| `marketBook` / `tickMarketBook` / `restockOnLoadForbidden` | **Read** stock and demand clamps. Fail if this package instant-refills, stores a second price, or retunes `PHASE8_MAGNITUDES` or `tickDrift`. A later buy/sell settles each ton at its post-move price, moves stock by one per ton, clamps stored price per ton, and lowers demand by one per sold ton. |
...
| Sell-back pays with the pod gone, or pays a consumed `saleId` again | Gate 10. |
...
| An N-ton buy moves stock by other than N, or one order takes one price step | Gate 13. |
| A same-world or mixed trip increases latinum, or a mixed trip leaves price stepped | Gate 14. |
...
`__BM1_PROBE__.commodityShipment` may expose `index`, `restore`, `snapshot`. Snapshot fields a scorer needs: `commodityNames`, `shipmentIds`, `podDigest` (so a probe can see the hold did not change), `purchaseRefused`, `cargoCap`, `worldCargoStatus`, `mode`, `lastAttemptReason`, `latinumDelta`, `standingDelta`, `alertsActive`, `inspectionCleared`, `customsCleared`, `contraband`, `wroteWorldCargo`, `prizeCargoCopied`, `lots`, `source`, `sales`, `soldByBook`, `marketPrice`, `marketStock`, `marketDemand`, `priceStep`, `priceFloor`, `priceCap`, `postMovePaid`, `demandAfterSell`, `sellAtFloorPaid`, `restockedToCap`, `tickDrift`, `sameWorldNet`, `mixedPathNet`, `priceReturned`, `saleConsumed`, `freeRepair`, `contactBookDigest`, `knownIncidentIds`, `flashQueued`, `briefingArchiveDigest`, `logBand`, `regionAllowsRefused`, `cultureFire`, `firingSolution`, `engagement_authorized`, `phase1TextUnchanged`, `roeModes`, `offersProtectAll`, `tractorIsBoarding`, `saveSlotCount`, `rowCapEnforced`, `mintedFromSave`, `bookInsideSystemStates`, `lockedFromRemastered`. This docs PR does not add the hook.
