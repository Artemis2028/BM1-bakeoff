...
| Finite markets? | **One market, the landed one.** Each world keeps limited stock and limited demand on `marketBook`. A later buy raises that world’s price and a later sell lowers it, by at most one capped step. Stock and demand do not refill in the trade or on load. Recovery is a later bounded drift, not a snap to cap. The commodity entry does not store a second price. This docs PR does not edit Phase 8 magnitudes. |
| Money machine? | **Closed.** Repeated buy/sell of the same good at the same world cannot mint unbounded credits. A two-world loop cannot farm profit, stock, or a free repair. |
...
| Phase 8 `marketBook`: clamped `stock` / `demand`, `applyShopBuy` / `applyShopSell` (stock ±1, price unchanged today), `tickMarketBook` refuses `restockToCap`, `restockOnLoadForbidden`, `jumpMustNotReprintInfinity` | Gates 1, 13, and 14. No second stock table. No instant refill. No unbounded round trip. Do not retune `PHASE8_MAGNITUDES` on this package. |
...
> Every good this book records has a source. A lot indexed from a player pod records `source: 'pod'`. A lot indexed from an accepted contract records `source: 'contract'`. A lot this book itself bought records `source: 'book-bought'`. This docs brief does not add the buy that would write `book-bought`. Indexing, a Phase 8 shop buy, world-cargo delivery, world-cargo expiry, tractor, capture, and salvage do not write `book-bought`. Tractoring a loose cargo pod does not make its goods sellable through this book. `tractorIsBoarding()` stays **false**. Tractor is not boarding, not capture, and not a sale. Sell-back through this book accepts only a lot the book itself sold: a `saleId` this book wrote at the time of that sale, with `soldByBook === true`. A tampered `soldByBook: true` on a pod or a shipment row, with no matching `saleId` in this book, restores as not sellable. Until the separate salvage/capture brief locks otherwise, boarding goods, prize goods, captured goods, and salvaged goods are not sellable through this book. A contract or offer with `contraband === true` keeps that mark on the lot. Sell-and-rebuy does not clear it. A transponder claim of `spoof` (`TRANSPONDER_MODES`) does not clear it and does not retag the lot `book-bought`. Routing the same tons through a friendly world or a friendly faction does not clear it and does not make the lot a legal book sale. This gate does not reopen Phase 8 `applyShopSell` / `removeCargoFromPods`, Phase 9.1 transponder, or Phase 9.2 spoof catch, and it does not add a prize-goods price. A later slice that pays latinum for a tractored loose pod, a prize hull’s cargo, a captured or salvaged lot, or any lot this book did not sell, or that clears `contraband` by a sale, a spoof, or a friendly dock, **fails**.

Lane owner (wording): **Number 2** on sell-back, provenance, and laundering; **Number Four** on `tractorIsBoarding() === false`.

**Pass:** tractor a loose pod whose `item` is a trade good. `tractorIsBoarding()` is false. The book’s `sales` map does not gain an id. The lot’s source is not `book-bought`. A sell-back of that good through the book pays 0 and does not clear the pod. A fixture sale id that this book wrote can be the only sell-back candidate; a pod that merely shares the name cannot. A `contraband: true` contract stays `contraband: true` after a sell-and-rebuy attempt, after a spoof claim, and after the player is at a friendly world. Captured and salvaged cargo pay 0 through this book.  
**Fail:** `tractorIsBoarding()` becomes true, a tractored, captured, salvaged, or prize lot becomes sellable through the book, a source is missing on a recorded lot, or `contraband` flips false because of a sale, a spoof, or a friendly route.
...
> Each world holds limited stock of each good and limited demand. That limit is the landed Phase 8 market row: `stock` and `demand` are clamped between the good’s floor and its `stockCap` / `demandCap`. This book does not publish a second stock list, a second demand list, or a price on the commodity entry. Indexing leaves `market.stock`, `market.demand`, and `market.price` unchanged. A buy this package later adds lowers that world’s stock and raises that world’s price. A sell raises that world’s stock, not past the cap, and lowers that world’s price. Each of those price moves is at most one capped step. The step, the caps, and the recovery amount are tunable config in this repository (gate 8). This brief does not choose the integers and does not edit `PHASE8_MAGNITUDES`. Landed `applyShopBuy` and `applyShopSell` today change stock by one unit and do not change `price`. This docs PR does not edit them. Stock and demand do not recover inside the trade, on load, or by snapping to cap. `tickMarketBook` still refuses `restockToCap`. `restockOnLoadForbidden` still holds (`lastLoadRestocked` is not set true by this package). Recovery, when a later tick moves stock or demand, is a bounded drift toward the middle of the clamp, not an instant refill. A later slice that gives a world infinite stock or demand, moves price by more than the configured step, refills stock or demand in the trade or on load, or stores a shop price on the commodity entry **fails**.

Lane owner (wording): **Number 2** on the price step; **Number Four** on the clamp and the no-refill rule.

**Pass:** after index, every market row’s stock, demand, and price are unchanged. If the later module exposes a buy of one unit, that world’s stock is lower, its price is higher by at most the configured step, and demand is still inside its cap. A sell moves price the other way and does not push stock above the cap. The next statement of the book does not set stock or demand to cap unless they were already there. Load does not restock.  
**Fail:** uncapped stock, a price jump larger than one configured step, an instant refill, a second price on the commodity entry, or a retune of `PHASE8_MAGNITUDES` to force the step.

### Hard gate 14 — No money machine

> No infinite credits from repeated buy and sell of the same good at the same world. No two-world loop that farms profit, stock, or free repairs. One trade may still move latinum by that world’s landed price. Repeating the round trip must not add that spread again without a bound, must not push stock above the cap, and must not refill stock that the first trade consumed. Buying at world A and selling at world B, then repeating, must not restore either world to cap and must not grant a repair. `evaluateDockService` still refuses repair and refuel when holding supply is unmet. `jumpMustNotReprintInfinity` still reports `freeRepair: false`. This package does not zero a repair price, does not call a repair from a commodity row, and does not raise `salvageLatinumCap` or `transportLatinumCap`. The credit bound, if a later slice needs a number for the round trip, is tunable config in this repository and is not copied from BM1-remastered (gate 8). A later slice that grows latinum on every same-world round trip, refills stock because the player flew to a second world and back, or grants a free repair from the loop **fails**.

Lane owner (wording): **Number 2**.

**Pass:** more same-world round trips than that market’s `stockCap` do not raise latinum without a bound and do not leave stock above the cap. A two-world loop of the same good does not snap either stock to cap and does not set a repair to free. `freeRepair` stays false.  
**Fail:** unbounded latinum from the loop, stock restored by the loop, or a repair price of 0 or a repair granted because of the trade.
...
| Every good records a source. Book-bought is tagged only by this book. Tractor, capture, salvage, and prize lots are not sellable here. Smuggled cargo is not laundered by sell-and-rebuy, spoof, or a friendly world. `tractorIsBoarding() === false` | Gate 10. |
...
| Finite per-world stock and demand. Buy raises price, sell lowers it, one capped step. No instant refill | Gate 13. |
| No same-world credit loop. No two-world ping-pong of profit, stock, or free repairs | Gate 14. |
...
The three maps each have one finite maximum (gate 6). The integer is config. It is not chosen here, not copied from remastered, and not the briefing cap of 24. Restore drops rows past the cap. It does not pay them and does not call `addCargoToPods` for them. A `latinum` or `credits` field on the book is dropped.
...
| Finite market | Read `marketBook` stock, demand, and price | Store a second price on the entry. Snap stock or demand to cap. Move price by more than one configured step. |
| Round trip | Nothing | Mint unbounded latinum, refill stock by ping-pong, or grant a free repair. |
...
- If a buy or sell is scoped, move the landed per-world price by at most one configured step and move stock the other way, inside the existing cap. Do not edit `PHASE8_MAGNITUDES`, `applyShopBuy`, or `applyShopSell`.
...
- No infinite stock or demand. No price step larger than the configured cap. No refill inside the trade or on load.
- No unbounded same-world round trip. No two-world loop that farms profit, stock, or a free repair.
...
| **S35.14** Finite market | Index does not move stock, demand, or price. A later buy raises price by at most one configured step and does not refill. A later sell lowers price and does not pass the stock cap. Load does not restock. | Gate 13. |
| **S35.15** No money machine | Same-world round trips past `stockCap` do not mint unbounded latinum. A two-world loop does not refill stock or grant a free repair. | Gate 14. |
...
| Q3 | What integer is the book’s maximum row count? | **The existence of a maximum is gate 6.** The integer is tunable config in this repo. It is not copied from remastered and it is not the briefing archive’s 24. This table does not pick it. Open shipments are not evicted ahead of that cap. |
