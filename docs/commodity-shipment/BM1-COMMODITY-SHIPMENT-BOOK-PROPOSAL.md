...
Commodity entries, shipment rows, lot rows, and sale rows each have one finite maximum (gate 6). The integer is config. It is not chosen here, not copied from remastered, and not the briefing cap of 24. Restore drops rows past the cap. It does not pay them and does not call `addCargoToPods` for them. A `latinum` or `credits` field on the book is dropped.
