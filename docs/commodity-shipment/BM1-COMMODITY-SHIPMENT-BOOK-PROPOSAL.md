...
| Finite markets? | **One per-ton settle rule on the landed market.** Each ton moves stock by one, steps the price once, and settles at that ton’s post-move price. An N-ton trade is N one-ton trades. A buy larger than the stock above the floor pays 0. A sell at the demand floor pays 0. Stored `market.price` is clamped per ton between a configured floor (≥ 1) and a configured cap. Book and Phase 8 shop use that same rule. `PHASE8_MAGNITUDES.tickDrift` stays 0. |
...
> Each world holds limited stock of each good and limited demand. That limit is the landed Phase 8 market row: `stock` and `demand` are clamped between the good’s floor and its `stockCap` / `demandCap`. This book does not publish a second stock list, a second demand list, or a price on the commodity entry. Indexing leaves `market.stock`, `market.demand`, and `market.price` unchanged. One settle rule covers every trade path at that world, the book and the Phase 8 shop, on the same stored `market.price`. The step is **per ton**, not per order. Each ton moves stock by one, steps the price once, and settles at that ton’s own post-move price. An N-ton trade is exactly N one-ton trades, which is how the Phase 8 shop already trades (one ton per call). A buy ton lowers stock by 1 and raises price by one configured step, then the buyer pays that post-move price. A sell ton raises stock by 1, lowers price by one configured step, and lowers demand by 1, then the seller receives that post-move price. Clamp applies on every ton: stored `market.price` stays between a configured floor (≥ 1) and a configured cap, in the same config as the step. At the floor or the cap the step may be partial; that ton still settles at the clamped price. A same-world buy-then-sell of the same tons therefore nets ≤ 0, whether the buy was split and the sell was one order or the buy was one order and the sells were split (gate 14). A buy of more tons than the stock above the floor is refused and pays 0: stock, demand, and price do not move. A sell of more tons than the demand above the floor is refused and pays 0 the same way. A sell that would push stock above `stockCap` is refused and pays 0. This brief does not choose the step, the floor, or the cap. Landed code only floors a price at 1 inside `dealResult` and on restore (`sanitizeMarket`); it does not clamp the live stored `market.price` to a cap, and `applyShopBuy` / `applyShopSell` today change stock by one ton and do not change `price`. This docs PR does not edit those functions or `PHASE8_MAGNITUDES`. A later engine slice may change their **settle** to this shared post-move per-ton rule and nothing else. `PHASE8_MAGNITUDES` stays unchanged. The Phase 8 suite is updated only for that settle. A later slice fails if the book steps once per order, if an N-ton buy moves stock by other than N, or if the shop still settles at the pre-move price while the book steps the same market. A book buy places that lot on its own empty pod slot. It does not stack those tons onto an untagged pod of the same good through `addCargoToPods`. If no empty slot remains, or the tons do not fit `cargoCap`, the purchase is refused and pays 0, and the market does not move. Stock and demand do not recover inside the trade, on load, or by snapping to cap. `tickMarketBook` still refuses `restockToCap`. `restockOnLoadForbidden` still holds. `PHASE8_MAGNITUDES.tickDrift` stays **0**. Recovery, when a later tick moves stock or demand, uses a new config value owned by this package, not `tickDrift`, and is a bounded drift, not an instant refill. A later slice that gives a world infinite stock or demand, steps price once for many tons, moves stock by other than one per ton, leaves stored price outside the floor and cap, pays a buy past the stock floor or a sell at the demand floor, refills stock or demand in the trade or on load, retunes `tickDrift` or `PHASE8_MAGNITUDES`, or stores a shop price on the commodity entry **fails**.
...
**Pass:** after index, every market row’s stock, demand, and price are unchanged. A buy of N tons lowers stock by N, steps price N times, clamps after each ton, and the latinum paid is the sum of those N post-move prices. A sell of N tons raises stock by N, lowers demand by N, steps price N times, and the latinum received is the sum of those N post-move prices. A buy of more tons than the stock above the floor pays 0 and leaves stock and price unchanged. The next sell after demand is at its floor pays 0 and leaves demand, stock, and price unchanged. A book buy occupies its own empty pod slot and does not add tons to an untagged pod of the same good. Load does not restock. `tickDrift` is still 0. `PHASE8_MAGNITUDES` is unchanged.  
**Fail:** an N-ton buy that moves stock by other than N, one price step for a multi-ton order, a lump settle at a single post-move price, a stored price outside the configured floor and cap, a paid buy past the stock floor, a paid sell at the demand floor, a shop path that does not use the same per-ton post-move price, an instant refill, a `tickDrift` or `PHASE8_MAGNITUDES` retune, or a second price on the commodity entry.

### Hard gate 14 — No money machine

> No per-trip gain on a same-world round trip, bounded or not. Each ton settles at its own post-move price (gate 13): a buy ton pays the raised price, a sell ton receives the lowered price, so a same-world buy-then-sell nets ≤ 0 by construction. Splitting the buy into one-ton trades and selling the tons in one order does not gain. Buying the tons in one order and selling them one ton at a time does not gain. That is not a capped positive spread. More same-world round trips than `stockCap` still net ≤ 0 in total, and no single one of them increases latinum. The same rule binds mixed paths on that world. Book buy then Phase 8 shop sell nets ≤ 0, and shop buy then book sell nets ≤ 0. After either mixed trip the stored price is the price from before the trip, when the fixture starts far enough from the price floor and the price cap that no ton’s step is partial. A trip that does hit the floor or the cap may end at the clamp; that edge is not the round-trip fixture, and the trip still nets ≤ 0. A book step that the shop does not reverse, or a shop trade that does not use the per-ton post-move price, is a price pump and **fails**. A two-world loop must not farm profit, stock, or a free repair, must not push stock above the cap, and must not refill stock the first trade consumed. `evaluateDockService` still refuses repair and refuel when holding supply is unmet. `jumpMustNotReprintInfinity` still reports `freeRepair: false`. This package does not zero a repair price, does not call a repair from a commodity row, and does not raise `salvageLatinumCap` or `transportLatinumCap`. A later slice that lets any same-world or mixed round trip increase latinum, leaves price a step away after a mixed trip, refills stock because the player flew to a second world and back, or grants a free repair from the loop **fails**.

Lane owner (wording): **Number 2**.

**Pass:** more same-world buy-then-sell trips than that market’s `stockCap` leave latinum no higher than at the start. Each of those trips nets ≤ 0. N one-ton buys followed by one N-ton sell nets ≤ 0. One N-ton buy followed by N one-ton sells nets ≤ 0. Those fixtures start strictly between the price floor and the price cap, with room for every step, and the stored price equals the pre-trip price. Book buy then shop sell nets ≤ 0 and the stored price equals the pre-trip price. Shop buy then book sell nets ≤ 0 and the stored price equals the pre-trip price. A two-world loop of the same good does not snap either stock to cap and does not set a repair to free. `freeRepair` stays false.  
**Fail:** any same-world, split-versus-lump, or mixed trip with a positive latinum delta, a mixed or split-versus-lump trip whose price does not return when it started clear of the clamp, stock restored by a two-world loop, or a repair price of 0 or a repair granted because of the trade.
...
| Finite per-world stock and demand. Each ton moves stock by one and settles at its own post-move price. An N-ton buy lowers stock by N. A buy past the stock floor pays 0. Stored price clamped per ton. Recovery config is this package’s, not `tickDrift` | Gate 13. |
| Same-world, split-versus-lump, and mixed round trips net ≤ 0. No per-trip gain. Price returns when the fixture starts clear of the price clamp. No two-world farm of profit, stock, or free repairs | Gate 14. |
...
commodityShipmentBook = {
  ...
  nextSaleId: 1,                 // monotonic; restore never lowers it
  selectedId: null,
...
`sales` is the only sell-back list (gate 10). An empty book has `sales` empty. A sale row names the player’s `book-bought` lot (`saleId`, `lotId`, `good`, `tons`). `saleId` comes from `nextSaleId`, a counter saved with the book. Issuing an id uses the current value and then increments. Restore never lowers `nextSaleId`. Dropping a row, including cap trimming, does not reuse that id. Sell-back pays only when a pod tagged with that lot still holds those tons, then removes them and consumes the `saleId`. Restore drops a sale row whose `soldByBook` is not true, drops a row with no matching tons aboard, and remembers a consumed `saleId` so a forged copy pays 0. Index, tractor, capture, and salvage do not insert a sale row. This brief does not add the action that would write one. A book buy, when a later slice adds one, writes the lot onto its own empty pod slot and does not stack it into untagged tons of the same good.
...
| Finite market | Read `marketBook` stock, demand, and price | Store a second price on the entry. Step once for many tons. Move stock by other than one per ton. Settle at the pre-move price. Pay a buy past the stock floor or a sell at the demand floor. Stack a book lot onto an untagged pod. Snap stock or demand to cap. Recover by retuning `tickDrift`. |
...
- If a buy or sell is scoped, each ton moves stock by one and settles at its own post-move price (gate 13). The engine slice **may** change `applyShopBuy` / `applyShopSell` settle behavior (Phase 8 from #31) to that shared post-move per-ton rule, and to nothing else. `PHASE8_MAGNITUDES` stays unchanged, including `tickDrift` at 0. The Phase 8 suite is updated only for the settle. The sentence that this docs PR does not edit those functions is only about **this** PR. It is not a lock against that later settle change. A book lot takes its own empty pod slot. `nextSaleId` is saved with the book and is not rewound when a row is dropped.
...
- No infinite stock or demand. No single price step for many tons. Stock moves by one per ton. Stored price stays inside the configured floor and cap, including a partial step at the edge. No buy paid past the stock floor. No sell paid at the demand floor. No refill inside the trade or on load. No recovery by editing `tickDrift`.
...
| **S35.14** Finite market | Index does not move stock, demand, or price. A buy of N tons lowers stock by N, steps price once per ton, and pays the sum of those post-move prices. The stored price stays inside the configured floor and cap after every ton. A buy of more tons than the stock above the floor pays 0 and leaves stock unchanged. A sell of N tons raises stock by N, lowers demand by N, and receives the sum of the per-ton post-move prices. The next sell after demand is at its floor pays 0. Load does not restock. Recovery does not change `tickDrift`. | Gate 13. |
| **S35.15** No money machine | More same-world buy-then-sell trips than `stockCap` each net ≤ 0. No per-trip gain. N one-ton buys then one N-ton sell nets ≤ 0. One N-ton buy then N one-ton sells nets ≤ 0. Book buy then shop sell, and shop buy then book sell, each net ≤ 0. Those fixtures start strictly between the price floor and the price cap, with room for every step, and the stored price ends where it started. A partial step is allowed only when a ton would pass the clamp; that edge is not these fixtures. A two-world loop does not refill stock or grant a free repair. | Gates 13 and 14. |
