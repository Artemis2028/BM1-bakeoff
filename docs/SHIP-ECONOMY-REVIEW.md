...
**Bake-off note:** content-only import. Pack + these docs landed; guided catalog
wire / `src/ship-economy.mjs` / live-economy probes were **not** applied.
Validation numbers below are from the guided remaster delivery. On bake-off,
confirm `node bm-ships/validate.mjs`. Reman **53** stays durable unlock + later
