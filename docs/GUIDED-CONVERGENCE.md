...
**Status:** docs-only product knowledge. Packages in this file are **engine-landed / stay locked** on bake-off `main` @ `e92db62` (after hygiene #69; prior tip `cb9be75` after Phase 10 roster engine PR #68). alertsActive, away-team XP, and the Phase 10 full-roster catalog are **closed / stay-locked**. Bajoran Solar Sailor ship brief is **open docs** under `docs/bajoran-solar-sailor/` (Flash “Bajoran Sail” is a **ship**, not a deferred utility). Named leftovers below are not Agreed next. Do not read the 12 September port as unimplemented work.  
**Repository:** `Artemis2028/BM1-bakeoff`  
**Planning baseline:** `9d130be` on `main` (12 September 2026). Hygiene vs `74f574b` (21 September 2026, after dockClear engine PR #61); continued vs `cb9be75` (23 September 2026, after S31 engine PR #68, tip `3ecadd3`); Sail reclass vs `e92db62` (24 September 2026, after hygiene #69).  
...
This file holds the detailed weapons table, boarding rules, standing tiers, and catalog/purchase notes. The development plan keeps the short **locked-package** list and pointers (§16.2). Rows that used to say “Agreed next” are **engine-landed / stay locked** on `main` @ `e92db62`. alertsActive (#63/#64), away-team XP (#65/#66), and the Phase 10 full-roster catalog (#67/#68) are **closed / stay-locked**. Flash “Bajoran Sail” is a **ship** brief (`docs/bajoran-solar-sailor/`), not a deferred-utility leftover. Named leftovers are not Agreed next. Warp Core stays deferred.

## Dual-track

| Track | What it already has | Do not treat as unfinished |
| --- | --- | --- |
| **Bake-off** (`BM1-bakeoff`) | Phase 1–5 **engine**, side-lane **engine** (`repairCapable` + arms overlay, Reman durable unlock, unrest → independence mint), additive `bm-ships/` pack + **catalog wire** (PR #28), Phase 6 **engine** (PR #24), Phase 7–9.4, boarding, Phase 10 Dominion-first, flags/utility, ledger, empty-armable, construction visuals, HTML catalogs, economy/difficulty, standing-tiers, dockClear S28, alertsActive S29 (#63/#64), away-team XP S30 (#65/#66), Phase 10 roster catalog S31 (#67/#68). Bajoran Solar Sailor **ship brief** is open docs (`docs/bajoran-solar-sailor/`), not engine. | Named **leftovers** only (below): flags capacity, **Warp Core** deferred utility, `protect-all` hold, Thaleron, Flash price locks, injectable odds / magnitudes. Bajoran Sail is **not** a deferred-utility leftover. Not a second catalog wire, not a reopen. The three former soft residuals are **stay-locked**. |
...
- Bajoran Sail / Bajoran Solar Sailor is **not** a deferred-utility leftover. Scoreable **ship** brief: `docs/bajoran-solar-sailor/` (docs-only; begins unarmed; no playable unlock; ledger §6.1 superseded; #48/#49 not reopened).
