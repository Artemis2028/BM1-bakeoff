...
**Status:** docs-only product knowledge. Packages in this file are **engine-landed / stay locked** on bake-off `main` @ `bc00a86` (header polish PR #77; S34 engine PR #75 @ `1b579e5`; S34 world cargo brief PR #74; S33 `briefingArchive` engine PR #73 @ `0534015`; briefing brief PR #72 @ `37bca25`; S32 Bajoran Solar Sailor engine PR #71 @ `557200f`; prior tip `e92db62` after hygiene #69; prior tip `cb9be75` after Phase 10 roster engine PR #68). alertsActive, away-team XP, the Phase 10 full-roster catalog, the sailor catalog, and the briefing archive are **closed / stay-locked**. World cargo delivery brief is **merged** @ `a76acbf`. The S34 `worldCargoDelivery` engine is **merged** [PR #75](https://github.com/Artemis2028/BM1-bakeoff/pull/75) @ `1b579e5`. Header polish #76 @ `7e8a27f` and #77 @ `bc00a86` are on `main`. The commodity and shipment book brief is **in review** under `docs/commodity-shipment/` (S35; docs only; does not relax S34). Named leftovers below are not Agreed next. Do not read the 12 September port as unimplemented work. **No Referee Pass claimed.**  
**Repository:** `Artemis2028/BM1-bakeoff`  
**Planning baseline:** `9d130be` on `main` (12 September 2026). Hygiene vs `74f574b` (21 September 2026, after dockClear engine PR #61); continued vs `cb9be75` (23 September 2026, after S31 engine PR #68, tip `3ecadd3`); Sail reclass vs `e92db62` (24 September 2026, after hygiene #69); S32 engine vs `557200f` (25 September 2026, after PR #71); S33 engine #73 @ `0534015`; S34 engine #75 is on `main` @ `1b579e5`. Current `main` is header polish #77 @ `bc00a86`. The commodity and shipment book brief is **in review** and does not relax S34.  
...
This file holds the detailed weapons table, boarding rules, standing tiers, and catalog/purchase notes. The development plan keeps the short **locked-package** list and pointers (§16.2). Rows that used to say “Agreed next” are **engine-landed / stay locked** on `main` @ `bc00a86`. alertsActive (#63/#64), away-team XP (#65/#66), the Phase 10 full-roster catalog (#67/#68), Bajoran Solar Sailor S32 (#70/#71 @ `557200f`), and the captains briefing archive (#72 @ `37bca25` / #73 @ `0534015`) are **closed / stay-locked**. Flash “Bajoran Sail” is a **ship** (`docs/bajoran-solar-sailor/`), not a deferred-utility leftover. World cargo delivery brief is **merged** @ `a76acbf`. The S34 engine is **merged** #75 @ `1b579e5`. The commodity and shipment book brief is **in review** (`docs/commodity-shipment/`). Named leftovers are not Agreed next. Warp Core stays deferred. **No Referee Pass claimed.**

## Dual-track

| Track | What it already has | Do not treat as unfinished |
| --- | --- | --- |
| **Bake-off** (`BM1-bakeoff`) | Phase 1–5 **engine**, side-lane **engine** (`repairCapable` + arms overlay, Reman durable unlock, unrest → independence mint), additive `bm-ships/` pack + **catalog wire** (PR #28), Phase 6 **engine** (PR #24), Phase 7–9.4, boarding, Phase 10 Dominion-first, flags/utility, ledger, empty-armable, construction visuals, HTML catalogs, economy/difficulty, standing-tiers, dockClear S28, alertsActive S29 (#63/#64), away-team XP S30 (#65/#66), Phase 10 roster catalog S31 (#67/#68), Bajoran Solar Sailor S32 (#70/#71 @ `557200f`). Captains briefing brief is **merged** (#72 @ `37bca25`). The S33 engine is **merged** (#73 @ `0534015`). World cargo delivery brief is **merged** (#74 @ `a76acbf`). The S34 engine is **merged** (#75 @ `1b579e5`). The commodity and shipment book brief is **in review** (docs only). | Named **leftovers** only (below): flags capacity, **Warp Core** deferred utility, `protect-all` hold, Thaleron, Flash price locks, injectable odds / magnitudes. Bajoran Sail is **not** a deferred-utility leftover. Not a second catalog wire, not a reopen. The three former soft residuals and the sailor catalog are **stay-locked**. |
...
13. **Bajoran Solar Sailor S32** — brief **#70 stay-locked** @ `d5e1c41`; engine **#71 stay-locked** @ `557200f` under `docs/bajoran-solar-sailor/` + `src/bajoran-solar-sailor.js`. Ship, unarmed, no playable unlock. **Stay locked.** The captains briefing archive (#72 @ `37bca25`, S33 engine #73 @ `0534015`) and the world cargo delivery brief (**merged** #74 @ `a76acbf`; S34 engine **merged** #75 @ `1b579e5`) do not reopen it. The commodity and shipment book brief does not reopen it either.
14. **Captains briefing / jump-intel archive S33** — brief **#72 stay-locked** @ `37bca25`; engine **#73 stay-locked** @ `0534015` under `docs/briefing-archive/` + `src/briefing-archive.js`. Knowledge-only. **Stay locked.** World cargo delivery is **merged** (#74 / #75) and does not reopen it. The commodity and shipment book brief does not reopen it either.
...
- World cargo delivery brief is **merged** under `docs/world-cargo-delivery/` (`worldCargoDelivery`, S34) [PR #74](https://github.com/Artemis2028/BM1-bakeoff/pull/74) @ `a76acbf`. The S34 engine is **merged** [PR #75](https://github.com/Artemis2028/BM1-bakeoff/pull/75) @ `1b579e5`. Nine hard gates: a cloaked drop never earns a legal payout; a failed cloaked drop of an open contract leaves the pods aboard; patrol suspicion is not a weapons grant; deliver-once holds across reload and re-dock; old saves have no deliveries pending. Not a Referee Pass. Does not reopen #33–#74. `WORLD_CARGO_LOCKED_FROM_REMASTERED` stays false.
- Commodity and shipment book brief is **in review** under [`docs/commodity-shipment/`](commodity-shipment/BM1-COMMODITY-SHIPMENT-BOOK-PROPOSAL.md) (`commodityShipmentBook`, S35). Docs only. A commodity entry is a name. A shipment record indexes an existing contract and does not own the hold. It does not relax S34 (`cloak-not-legal`, mode-trust restore, `completionToken` restores as delivered). No standing write. No prize-goods sale (that alt item needs its own capture-rules brief). Trade does not write or clear reports or FLASH. Tractoring a loose pod does not make it sellable, and `tractorIsBoarding()` stays false. Dominion trade access is pack `dominion-all` / `dominion-core`. No culture fire and no Phase 1 rewrite. The same list also locks one shared hold, pay-once delivery, a fixed save cap with no minted credits, finite per-world stock and a capped price step, no round-trip money machine, provenance with no contraband laundering, and a UI fit that does not cover the campaign readout, world cargo, or the dock. Prices and limits stay tunable config in this repo. `COMMODITY_SHIPMENT_LOCKED_FROM_REMASTERED` stays false. Not a Referee Pass. Does not reopen #33–#77.
