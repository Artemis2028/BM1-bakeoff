...
- Commodity and shipment book brief is **in review** under [`docs/commodity-shipment/`](commodity-shipment/BM1-COMMODITY-SHIPMENT-BOOK-PROPOSAL.md) (`commodityShipmentBook`, S35). Docs only. A commodity entry is a name. A shipment record indexes an existing contract and does not own the hold. It does not relax S34 (`cloak-not-legal`, mode-trust restore, `completionToken` restores as delivered). No standing write. No prize-goods sale (that alt item needs its own capture-rules brief). `COMMODITY_SHIPMENT_LOCKED_FROM_REMASTERED` stays false. Not a Referee Pass. Does not reopen #33–#77.
