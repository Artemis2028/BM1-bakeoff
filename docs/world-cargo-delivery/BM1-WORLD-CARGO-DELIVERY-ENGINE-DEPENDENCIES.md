...
| Covert drop while `isHullCloaked` | A customs stamp, a cleared checkpoint, or a standing write |
...
**Gap this brief closes (docs now; module only if scoped):** there is no **scoreable S34 contract** that completes freight at the destination world, refuses station hail/dock, and lets a cloaked drop finish a covert contract without clearing inspection, contraband, or standing.
...
| Optional `scripts/test-world-cargo-delivery.mjs` | Offline: world versus station, cloak versus customs, no authority, payout once, old-save empty, lock false. |
...
| `creditWorthwhileTrip` | **Open only,** token `world-cargo:${id}`, callback the existing +3. Fail if covert calls `adjustFactionStanding`. |
...
| `isHullCloaked` | **Read.** Fail if silent running counts as covert, or if open delivery succeeds while cloaked. |
...
| Covert drop sets `compliance_verified` or `access_clearance` | Gate 2. That is customs. |
| Covert drop calls the +3 callback | Gates 2 and 3. |
| `restoreMissingContractCargo` reloads a delivered book pod | Gate 4. A second pay, or a partial that refills itself. |
| Book stored on `systemStates` | Gate 5. Load wipes it and can strand or revive a token. |
| Outcome text clipped, or no covert shot | Gate 6. Header strip is not a substitute and not a defect of this package. |
| Editing `deliverContractIfPossible` payout math for legacy pods | Gate 7. The allowed edit is the book-id skip only. |
| Copying a remastered freight id or flipping a lock flag | Gate 8. |

## Probe hook sketch (later)

`__BM1_PROBE__.worldCargo` may expose `evaluate`, `complete`, `drop`, `noteStation`, `expire`, `snapshot`. Snapshot fields a scorer needs: `atWorld`, `status`, `reason`, `latinumDelta`, `standingDelta`, `inspectionCleared`, `customsCleared`, `contraband`, `encounterFacts`, `roeModes`, `offersProtectAll`, `completionToken`, `saveSlotCount`, `bookInsideSystemStates`. This docs PR does not add the hook.
