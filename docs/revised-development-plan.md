...
- Commodity and shipment book brief is **in review** under [`docs/commodity-shipment/`](commodity-shipment/BM1-COMMODITY-SHIPMENT-BOOK-PROPOSAL.md) (package `commodityShipmentBook`, slice S35). Docs only. Twelve hard gates. It indexes commodity names and shipment records. It does not own the hold, does not complete freight, does not write standing, and does not sell captured or prize goods. It does not relax `cloak-not-legal`, mode-trust restore, or `completionToken` → `delivered`. Trade does not touch reports or FLASH. A tractored loose pod is not sellable through the book. Dominion access stays pack `dominion-all` / `dominion-core`. No culture gift and no Phase 1 rewrite. Not a Referee Pass. Does not reopen #33–#77. `COMMODITY_SHIPMENT_LOCKED_FROM_REMASTERED` stays false.
