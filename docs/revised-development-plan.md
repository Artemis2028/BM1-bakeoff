...
Updated: 26 September 2026 (current `main` @ `bc00a86` after header polish PR #77; S34 `worldCargoDelivery` engine **merged** PR #75 @ `1b579e5`; S34 world cargo brief PR #74 @ `a76acbf`; commodity and shipment book brief **in review** under `docs/commodity-shipment/`, docs only; S33 `briefingArchive` engine PR #73 @ `0534015`; captains briefing brief PR #72 @ `37bca25`; S32 Bajoran Solar Sailor engine stays PR #71 @ `557200f`; prior Sail ship brief 24 September 2026 vs `e92db62` after hygiene #69; prior hygiene 23 September 2026 vs `cb9be75` after Phase 10 roster engine PR #68, tip `3ecadd3`; prior hygiene 21 September 2026 vs `74f574b` after dockClear engine PR #61).  
Checkpoint: Phase 1–10 Dominion-first, side-lane, catalog wire, EW 9–9.4, boarding, flags/utility, ledger, empty-armable, construction, HTML catalogs, economy/difficulty, standing-tiers, dockClear S28, alertsActive S29 (#63/#64), away-team XP S30 (#65/#66), the Phase 10 roster catalog S31 (#67/#68), and Bajoran Solar Sailor S32 (#70/#71 @ `557200f`) are on bake-off `main`. Phase 6 sensors/cloak engine landed [PR #24](https://github.com/Artemis2028/BM1-bakeoff/pull/24) @ `749393b`. Bajoran Solar Sailor is a **stay-locked** ship catalog, not a deferred-utility leftover. The captains briefing archive brief is **merged** @ `37bca25`. The S33 engine is **merged** @ `0534015`. The world cargo delivery brief is **merged** @ `a76acbf` (PR #74). The S34 engine is **merged** @ `1b579e5` (PR #75). The commodity and shipment book brief is **in review** and does not relax those S34 rules. **No Referee Pass claimed** on the post-Phase-4 packages. Product knowledge lives in **§16** and `docs/GUIDED-CONVERGENCE.md`. Do not treat Passed Phase 1–6 briefs, the side-lane, or later locked engines as incomplete.

This is the current roadmap for what we have completed, what we need to add, and what we would like to build later. It carries forward the cloud handoff and faction-doctrine work, with the decisions made during the Phase 1 and Phase 2 reviews taking precedence. The later milestones were proposed implementation slices, not approval to implement every idea at once. Sections 3–15 retain the original design text; **§16 is the bake-off-relative status** and takes precedence when older “Pending / next / Agreed next” lines disagree with landed Passes. Hygiene 21 September 2026 flipped the §2 holding-zones **Pending** row and the §16.2 Phase 10 / catalog-not-wired claims. Hygiene 23 September 2026 marks alertsActive, away-team XP, and the Phase 10 full-roster catalog **closed / stay-locked** (#63–#68). The 24 September 2026 ship brief removes Bajoran Sail from the deferred-utility leftover; Warp Core stays deferred. S32 engine #71 is on `main` @ `557200f`. The 25 September 2026 captains briefing brief is **merged** [PR #72](https://github.com/Artemis2028/BM1-bakeoff/pull/72) @ `37bca25`. The S33 engine is **merged** [PR #73](https://github.com/Artemis2028/BM1-bakeoff/pull/73) @ `0534015`. The world cargo delivery brief is **merged** #74 @ `a76acbf`. The S34 engine is **merged** #75 @ `1b579e5`. The commodity and shipment book brief is **in review** and does not reopen #33–#77. **No Referee Pass claimed.**
...
| Holding zones, access enforcement, incident escalation and FLASH alerts | **Landed** — Phase 3 engine [PR #8](https://github.com/Artemis2028/BM1-bakeoff/pull/8) / [PR #9](https://github.com/Artemis2028/BM1-bakeoff/pull/9); Phase 4 engine [PR #13](https://github.com/Artemis2028/BM1-bakeoff/pull/13). Older “Pending” checkpoint text is **stale**. Not a reopen. | Holding / compliance / incidents / FLASH are on `main`. `alertsActive` readout **closed / stay-locked** (brief #63 @ `60c2f69`; S29 engine #64 @ `ce177f0`; not a Referee Pass). |
| Broader economy, sensors, fleet coordination and Dominion campaign | **Engines landed** on bake-off (see §16). Older “requested direction” is **stale checkpoint text**, not a reopen. | Phase 6 PR #24; Phase 7 PR #29; Phase 8 PR #31 + economy #56/#57; Phase 10 Dominion-first #40/#41; full-roster catalog **stay-locked** #67/#68 (S18.18 unamended). |
...
Historical 11 September instruction (superseded): before the next engine branch, confirm the actual pushed head contains this reviewed pair or its equivalent. Keep earlier audits as historical evidence; revalidate their unresolved findings against that head. Bake-off `main` @ `cb9be75` already contains Phase 2 and later locked engines through #68.
...
`access` and `alerts` were reserved data at the 11 September checkpoint. Phase 3 (PR #8 / #9) implements holding / access / compliance; Phase 4 (PR #13) implements incidents / FLASH. Do **not** present those as still reserved. `alertsActive` readout is **closed / stay-locked** (brief #63 @ `60c2f69`; S29 engine #64 @ `ce177f0`; not a Referee Pass). `protect-all` remains on **hold** until the engine can distinguish an aggressor from someone returning fire. Do not invent a third ROE.
...
Later decisions to retain: default BM1 versus optional BM2-derived roster; upgrade pacing and maintenance; fleet controls and ships left behind; sensor presentation; scope of lawful trade exceptions; Blender's legal claim; hidden-region revelation and invasion timing; weapon roles and shield exceptions. Do not reopen the settled two-mode Phase 2 ROE merely to match older terminology. Closed and **stay-locked** (not open, not a Referee Pass): `alertsActive` S29 (#63 @ `60c2f69` / #64 @ `ce177f0`); away-team XP S30 `named_mix` (#65 @ `73b2963` / #66 @ `10540aa`); Phase 10 full-roster catalog (#67 @ `17063c4` / #68 tip `3ecadd3` merged at `cb9be75`; S18.18 unamended). Leftovers that are **not** Agreed-next: magnitudes injectable; flags capacity / activation TBD; **Warp Core** deferred utility (separate); `protect-all` hold; Thaleron unverified; Flash prices not live locks; boarding success-odds injectable. Bajoran Sail is a **ship** brief (`docs/bajoran-solar-sailor/`), not a deferred utility.
...
The 21 September 2026 hygiene pass aligns §2 / §16 with `main` @ `74f574b` (after dockClear engine PR #61). The 23 September 2026 pass continues that alignment through `main` @ `cb9be75` (after roster engine PR #68). The 24 September 2026 Bajoran Solar Sailor brief continues from `main` @ `e92db62` (after hygiene #69). The 25 September 2026 pass records S32 engine #71 on `main` @ `557200f`, records briefing brief #72 @ `37bca25`, records S33 engine #73 on `main` @ `0534015`, and records the world cargo delivery brief **merged** #74 @ `a76acbf` with the S34 engine **merged** #75 @ `1b579e5`. The 26 September 2026 commodity and shipment book brief is **in review** under `docs/commodity-shipment/` and does not replace this plan, does not mark Warp Core or the other leftovers done, does not claim a Referee Pass, and does not reopen Phase 1–10 / EW / boarding / utility / ledger / empty-armable / construction / HTML / economy / standing / dockClear / alertsActive / away-team XP / roster-catalog / hygiene / sailor / briefing / world-cargo / header-strip gates (#33–#77).

## 16. Convergence backlog (from guided remastered-work roadmap)

Added: 12 September 2026. Hygiene: 21 September 2026 vs `main` @ `74f574b`; continued 23 September 2026 vs `main` @ `cb9be75`; Sail reclass 24 September 2026 vs `main` @ `e92db62`; S32 engine 25 September 2026 vs `main` @ `557200f`; S33 engine #73 is on `main` @ `0534015`. Current `main` is header polish #77 @ `bc00a86` (S34 engine #75 @ `1b579e5`). **Docs only** for this plan file. Detail, Flash weapon table, boarding rules, and standing numbers live in [`docs/GUIDED-CONVERGENCE.md`](GUIDED-CONVERGENCE.md). Bajoran Solar Sailor: [`docs/bajoran-solar-sailor/`](bajoran-solar-sailor/BM1-BAJORAN-SOLAR-SAILOR-PROPOSAL.md) (S32 engine #71 stay-locked @ `557200f`). Captains briefing archive brief **merged** #72; S33 engine **merged** #73: [`docs/briefing-archive/`](briefing-archive/BM1-BRIEFING-ARCHIVE-PROPOSAL.md). World cargo delivery brief **merged** #74; S34 engine **merged** #75 @ `1b579e5`: [`docs/world-cargo-delivery/`](world-cargo-delivery/BM1-WORLD-CARGO-DELIVERY-PROPOSAL.md). Commodity and shipment book brief **in review**: [`docs/commodity-shipment/`](commodity-shipment/BM1-COMMODITY-SHIPMENT-BOOK-PROPOSAL.md). **No Referee Pass claimed.**

**Dual-track:** guided `BM1-remastered-work` may have numbered catalog wire and economy/standing differently. Bake-off already has Phase 1–10 Dominion-first engine, the side-lane, additive `bm-ships/` **and catalog wire (PR #28)**, EW 9–9.4, boarding, flags/utility, ledger, empty-armable, construction, HTML catalogs, economy/difficulty, standing-tiers, dockClear S28, alertsActive S29 (#63/#64), away-team XP S30 (#65/#66), the Phase 10 roster catalog S31 (#67/#68), and Bajoran Solar Sailor S32 (#70/#71 @ `557200f`). Current `main` is header polish #77 @ `bc00a86`, with the S34 engine #75 @ `1b579e5`. Do not crib guided engine code. The commodity and shipment book brief is **in review** and is not a second implementation of those packages, and it does not relax S34. **No Referee Pass claimed.**
...
| Phase 4 incidents / escalation / alerts | **Passed**. `alertsActive` readout **closed / stay-locked** (#63 @ `60c2f69` / #64 @ `ce177f0`). | §6; `docs/phase4/`; `docs/alerts-active/`; PR #13 |
...
Rows below are **locked** on `main` @ `bc00a86` (through S32 engine #71 @ `557200f`, briefing brief #72, S33 engine #73, world cargo brief #74, S34 engine #75 @ `1b579e5`, and header polish #76 / #77). They are **not** Agreed next. Do not reopen closed gates. #33–#77 stay locked. The commodity and shipment book brief is **in review**, not a row in this locked table. **No Referee Pass claimed.**

| # | Package | Status on bake-off | Class | Overlap / pointer |
| --- | --- | --- | --- | --- |
| 1 | Weapon / device source ledger | **Engine landed** (`docs/weapon-ledger/`; PR #48 brief / PR #49 engine @ `a3d9611`). **Stay locked** — do not reopen from empty-armable, construction visuals, HTML catalogs, or the Sail ship brief. | **Locked** | §11 / §13 BM1–BM2 audit. Flash prices are **source material, not final prices**. Three disruptor identities. Tractor stays a slot. **Warp Core** stays an **explicitly deferred utility**. Flash Bajoran Sail §6.1 utility call is **superseded** by `docs/bajoran-solar-sailor/` (ship; one-line note only). Phase 9 matrix **read-only**. **Not** a reopen of EW / boarding / Phase 10 / flags. |
...
| 10 | Dominion distribution / Gorn reserved / major-threat mission-only + hidden Dominion campaign | **Engine landed** (`docs/phase10/`; PR #40 brief / PR #41 engine @ `2ad94b7`). Catalog helpers wired (PR #28). Live spawn/purchase + hide + campaign book **implemented** (S18). Full-roster sibling catalog **closed / stay-locked** (`docs/phase10-roster/`; brief #67 @ `17063c4`; S31 engine #68 tip `3ecadd3` merged at `cb9be75`). **Stay locked.** | **Locked** — Dominion-first; roster catalog **stay-locked** | Doctrine Gorn absence + hidden Dominion campaign (§12). Pack `reserved-gorn`, `dominion-all` / `dominion-core`, `mission-only`. Sibling `factionRosterBook`; S18.18 unamended (`scope: 'dominion-first'`, `rosterPlayable` false); paths ≠ ROE; knowledge default `none`; no gifted FS / culture / `engagement_authorized`. Short write-up: `GUIDED-CONVERGENCE.md` §10. Magnitudes / discovery % / invasion odds **TBD / injectable**. **Not** a reopen of EW or boarding. Older “brief open / spawn not implemented / full roster deferred” copy is **stale**. |
| 11 | DockClear / UI-fit polish | **Engine landed** (`docs/dock-clear/`; PR #60 brief @ `3bdc18a` / PR #61 engine @ `74f574b`). **Stay locked.** | **Locked** | Fit only — target / map / related dock chrome. No gameplay rewrite. Does **not** reopen GUIDED §6 / #33–#68. `DOCK_CLEAR_LOCKED_FROM_REMASTERED` stays false. `alertsActive` readout is **closed** in #63/#64 and **out** of this lane. |
| 12 | Bajoran Solar Sailor | **Engine landed** (`docs/bajoran-solar-sailor/`; PR #70 brief @ `d5e1c41` / PR #71 engine @ `557200f`). **Stay locked.** | **Locked** | Ship, not a deferred utility. Begins unarmed. No `rosterPlayable` gift. Id 345 is not a live lock. `BAJORAN_SOLAR_SAILOR_LOCKED_FROM_REMASTERED` stays false. Warp Core stays deferred. The briefing archive does **not** reopen this lane. |
| 13 | Captains briefing / jump-intel archive | **Engine landed** (`docs/briefing-archive/`; PR #72 brief @ `37bca25` / PR #73 engine @ `0534015`). **Stay locked.** | **Locked** | Knowledge-only. Cap 24 × 12. `BRIEFING_ARCHIVE_LOCKED_FROM_REMASTERED` stays false. World cargo delivery is **merged** (#74 / #75) and does **not** reopen this lane. The commodity and shipment book brief does **not** reopen it either. |
...
6. Boarding / capture / command transfer — engine landed (`docs/boarding/`; PRs #38/#39). Away-team XP **closed / stay-locked** (brief **#65** @ `73b2963`; S30 engine **#66** @ `10540aa`, `named_mix`). **Stay locked.**
...
11. Phase 10 Dominion-first — engine landed (`docs/phase10/`; PRs #40/#41). Full-roster sibling catalog **closed / stay-locked** (brief **#67** @ `17063c4`; S31 engine **#68** tip `3ecadd3` merged at `cb9be75`; S18.18 unamended). **Stay locked.**
12. DockClear / UI-fit S28 — brief **#60 stay-locked**; engine **#61 stay-locked** @ `74f574b` under `docs/dock-clear/`.
13. Bajoran Solar Sailor S32 — brief **#70 stay-locked** @ `d5e1c41`; engine **#71 stay-locked** @ `557200f` under `docs/bajoran-solar-sailor/` + `src/bajoran-solar-sailor.js`. Ship, unarmed, no `rosterPlayable` gift. `BAJORAN_SOLAR_SAILOR_LOCKED_FROM_REMASTERED` stays false.
14. Captains briefing / jump-intel archive S33 — brief **#72 stay-locked** @ `37bca25`; engine **#73 stay-locked** @ `0534015` under `docs/briefing-archive/` + `src/briefing-archive.js`. Knowledge-only. Cap 24 × 12. `BRIEFING_ARCHIVE_LOCKED_FROM_REMASTERED` stays false. The world cargo brief does **not** reopen this lane.
...
- Do not reopen Phase 1–6 briefs, the side-lane, EW, boarding, Phase 10, flags/utility, ledger, empty-armable, construction, HTML, economy, standing, dockClear, alertsActive, away-team XP, the roster catalog, the sailor catalog, or the briefing archive as incomplete. **#33–#77 stay locked.** The S34 engine is **merged** #75 @ `1b579e5`. The commodity and shipment book brief is **in review**, not a locked engine row.
- Construction visuals **engine** landed (`docs/construction-visuals/`; PRs #52/#53) — stay locked.
- Closed soft residuals (**stay-locked**, not open): `alertsActive` S29 (#63 @ `60c2f69` / #64 @ `ce177f0`); away-team XP S30 `named_mix` (#65 @ `73b2963` / #66 @ `10540aa`); Phase 10 full-roster catalog (#67 @ `17063c4` / #68 tip `3ecadd3` merged at `cb9be75`; S18.18 unamended; `rosterPlayable` false; paths ≠ ROE; knowledge default `none`). Remastered-lock flags stay false. **No Referee Pass claimed.**
- Leftovers stay explicit and are **not** Agreed-next packages: magnitudes / discovery % / invasion odds injectable; flags capacity / activation TBD (#46/#47); **Warp Core** deferred utility (ledger §6.2; not this ship brief); `protect-all` ROE hold (do not invent a third ROE); Thaleron facility unverified; Flash prices not live locks; boarding success-odds tables injectable.
- Bajoran Sail / Bajoran Solar Sailor is **not** a deferred-utility leftover. S32 engine **stay-locked** [PR #71](https://github.com/Artemis2028/BM1-bakeoff/pull/71) @ `557200f` (brief #70 @ `d5e1c41`; room lock **3a**; begins unarmed; knowledge/catalog only; no `rosterPlayable` gift; ledger §6.1 superseded; #48/#49 not reopened; id 345 not a live lock). **No Referee Pass claimed.**
- Captains briefing / jump-intel archive brief is **merged** [PR #72](https://github.com/Artemis2028/BM1-bakeoff/pull/72) @ `37bca25` (opened from `main` @ `557200f`). S33 `briefingArchive` engine is **merged** [PR #73](https://github.com/Artemis2028/BM1-bakeoff/pull/73) @ `0534015`. Not a Referee Pass. #33–#73 stay locked. S32 stays @ `557200f`. `BRIEFING_ARCHIVE_LOCKED_FROM_REMASTERED` stays false.
- World cargo delivery brief is **merged** [PR #74](https://github.com/Artemis2028/BM1-bakeoff/pull/74) @ `a76acbf` under [`docs/world-cargo-delivery/`](world-cargo-delivery/BM1-WORLD-CARGO-DELIVERY-PROPOSAL.md) (package `worldCargoDelivery`, slice S34). The S34 engine is **merged** [PR #75](https://github.com/Artemis2028/BM1-bakeoff/pull/75) @ `1b579e5`. Nine hard gates: a cloaked drop never earns a legal payout; a failed cloaked drop of an open contract leaves the pods aboard; patrol suspicion is not a weapons grant; deliver-once holds across reload and re-dock; old saves have no deliveries pending. Not a Referee Pass. Does not reopen #33–#74. `WORLD_CARGO_LOCKED_FROM_REMASTERED` stays false.
- Commodity and shipment book brief is **in review** under [`docs/commodity-shipment/`](commodity-shipment/BM1-COMMODITY-SHIPMENT-BOOK-PROPOSAL.md) (package `commodityShipmentBook`, slice S35). Docs only. Fourteen hard gates. It indexes commodity names and shipment records. It does not own the hold, does not complete freight, does not write standing, and does not sell captured or prize goods. It does not relax `cloak-not-legal`, mode-trust restore, or `completionToken` → `delivered`. Trade does not touch reports or FLASH. A tractored loose pod is not sellable through the book. Dominion access stays pack `dominion-all` / `dominion-core`. No culture gift and no Phase 1 rewrite. The same list locks one shared hold, pay-once delivery, a fixed save maximum with no minted goods or credits, finite per-world markets and a capped price step, no money machine, and provenance that does not launder contraband. Not a Referee Pass. Does not reopen #33–#77. `COMMODITY_SHIPMENT_LOCKED_FROM_REMASTERED` stays false.
