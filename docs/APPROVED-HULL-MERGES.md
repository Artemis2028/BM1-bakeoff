...
**Bake-off note:** this file is content extracted from the guided full-roster-v2
patch. `bm-ships/ships.json` is the authored roster. Bake-off did **not** import
`data/starship_manifest.json`, `scripts/sync-ship-roster.mjs`, or catalog wire —
live traffic/yards still use the pre-pack 66-hull path. Alias map: old → survivor
IDs in the table below, and machine-readable as `bm-ships/ships.json` `aliases`
and `bm-ships/integration-rules.json` `hullAliases`. Reman hull **53 / `bm-ship:53`**
is not a merge; access stays durable unlock + later `meetPackPurchaseDecision`.
...
`bm-ships/ships.json` is the authored roster. Guided remaster generates
`data/starship_manifest.json` from it; that sync path is **not** on bake-off
until catalog wire. Do not edit a second hull database here.
