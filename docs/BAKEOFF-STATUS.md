...
Updated: `2026-09-13T20:45:00Z` (Phase 9 EW / weapons brief on `cursor/phase9-ew-weapons-brief-68c8`)  
Head: docs-only Phase 9 proposal from `main` @ `02addb6` (after Phase 8 engine PR #31). **No Referee Pass claimed.**
...
| `docs/phase9/` | Phase 9 EW / separately reviewed weapon behavior proposal + engine deps — brief open; EW on P6 layers + P6.5 reserved `ew` budget; ghosts = contact-book rows; jamming cannot unsend delivered P4/P5 reports; weapons matrix before overhaul (Flash table = source, not prices); pursuit ≠ permission ≠ per-weapon gate; boarding out; **no Referee Pass claimed** |
...
18. **Phase 9 brief (EW ghosts/jamming / weapons matrix / fire gates)** — **proposal** from `main` @ `02addb6` (after Phase 8 engine PR #31). Six hard gates for scoring (EW on P6 contact layers + P6.5 reserved `ew` consumer; ghosts = contact-book rows only, no fake hulls, no gifted `firingSolution`; jamming cannot wipe/unsend an already-delivered P4/P5 report; weapons matrix before any overhaul — family/range/arc/tracking/shield/hull/energy/counters/faction + BM1/BM2/Trek/new provenance; Flash prices are source material not final; pursuit ≠ permission to engage ≠ per-weapon firing gate, no culture / `engagement_authorized` inject; boarding stays out). Each EW family needs cost, duration, counter, attributable consequence. **Cites** Phase 6 layers, Phase 6.5 reserved consumer, Phase 4/5 reports; does **not** reopen them. **No Referee Pass claimed.** Referee / One score the six gates **before** any engine PR. Number 2 scores gates 2, 3, 5 (ghost / report / fire-gate wording). Number Four scores gates 1, 4 (layers+power + matrix-before-overhaul). Number Three probes only after engine (S14 sketch).
