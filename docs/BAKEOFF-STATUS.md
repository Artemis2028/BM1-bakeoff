...
Updated: `2026-09-14T11:30:00Z` (Phase 9.1 robust-EW brief on `cursor/phase91-ew-robustness-brief-f25d`)  
Head: Phase 9 engine merged on `main` @ `2b38a14` (PR #33). Phase 9.1 amend brief **open** (docs only). **Keep #33 locked.** **No Referee Pass claimed.**
...
| `docs/phase9/` | Phase 9 EW / separately reviewed weapon behavior proposal + engine deps — brief merged (PR #32); engine implemented from that brief (PR #33, `2b38a14`); EW on P6 layers + P6.5 reserved `ew` budget; ghosts = contact-book rows; jamming cannot unsend delivered P4/P5 reports; weapons matrix before overhaul (Flash table = source, not prices); pursuit ≠ permission ≠ per-weapon gate; boarding out. **#33 stays locked** (do not reopen the six Phase 9 gates). Phase **9.1 amend** brief (robust EW / burn-through / residue / RSS / HoJ matrix row / transponder claim) is **open** on this tip — companions `BM1-PHASE9.1-EW-ROBUSTNESS-PROPOSAL.md` + `BM1-PHASE9.1-ENGINE-DEPENDENCIES.md`; magnitudes injectable; remastered EW is background shapes only (no `git am`); **no Referee Pass claimed** |
...
20. **Phase 9.1 brief (robust EW / burn-through / HoJ / transponder claim)** — **proposal** from `main` @ `2b38a14` (after Phase 9 engine PR #33). Seven hard gates for scoring (spend-to-suppress on reserved `ew` + dedicated slot; burn-through `B × √Q` always available for a funded receiver; residue-before-void; RSS all-paid emitters + true-side friendly labels; HoJ as a weapons-matrix row first with silence→coast and no gifted FS / `engagement_authorized`; transponder On/Off/Spoof = claim layer only, never Phase 1 identity; preserve Phase 9 / #33). Magnitudes injectable / TBD. Remastered EW pack is background shapes only — do not `git am`, do not lock remastered watts. **Does not** reopen Phase 9 six gates. **No Referee Pass claimed.** Referee / One score the seven gates before any engine PR. Number 2 scores residue / claim / fire-gate wording. Number Four scores `ew` draw, contest, slot, matrix row. Number Three probes only after engine.

## Version history

Newest first.

- **Phase 9.1 brief** — `2026-09-14T11:30:00Z` (Asia/Dubai `2026-09-14T15:30:00+04:00`) — docs-only 9.1 amend proposal + engine-deps from `main` @ `2b38a14` (after PR #33). Seven locks (spend-to-suppress on reserved `ew`; burn-through always available; residue-before-void; RSS + true-side labels; HoJ matrix row first; transponder claim only; preserve #33). S15 sketch. Magnitudes injectable. No `git am` remastered. No Referee Pass claimed. **#33 stays locked.**
