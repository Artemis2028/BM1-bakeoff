...
Updated: `2026-09-13T21:20:00Z` (Phase 9 EW / weapons engine on `cursor/phase9-ew-weapons-engine-1d87`)  
Head: Phase 9 engine from `main` @ `b9f4385` (after Phase 9 brief PR #32). **No Referee Pass claimed.**
...
| `docs/phase9/` | Phase 9 EW / separately reviewed weapon behavior proposal + engine deps — brief merged (PR #32); engine implemented from that brief on this tip; EW on P6 layers + P6.5 reserved `ew` budget; ghosts = contact-book rows; jamming cannot unsend delivered P4/P5 reports; weapons matrix before overhaul (Flash table = source, not prices); pursuit ≠ permission ≠ per-weapon gate; boarding out; **no Referee Pass claimed** |
...
19. **Phase 9 engine (EW ghosts/jamming / weapons matrix / fire gates)** — implemented from `docs/phase9/` only on this tip (from `main` @ `b9f4385`). Turns the reserved Phase 6.5 `ew` consumer on; ghosts are contact-book rows; jamming decays live layers only; read-only ten-column weapons matrix + mapping (no combat retune); fire gates stay three-step; boarding stays out. S14. **Does not** reopen Phase 6 layers, Phase 6.5 generation, Phase 4/5 delivered reports, or catalog wire. Magnitudes injectable / TBD. **No Referee Pass claimed.**

## Version history

Newest first.

- **Phase 9 engine** — `2026-09-13T21:20:00Z` (Asia/Dubai `2026-09-14T01:20:00+04:00`) — EW ghosts/jamming, weapons matrix, fire gates from `docs/phase9/` on `cursor/phase9-ew-weapons-engine-1d87` (`main` @ `b9f4385`). S14. No Referee Pass claimed.
