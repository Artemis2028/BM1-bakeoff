...
Updated: `2026-09-14T12:30:00Z` (Phase 9.1 robust-EW engine on `cursor/phase91-ew-robustness-engine-8db5`)  
Head: Phase 9 engine merged on `main` @ `2b38a14` (PR #33). Phase 9.1 amend brief merged [PR #34](https://github.com/Artemis2028/BM1-bakeoff/pull/34) @ `4d038e7`. Phase 9.1 **engine** in flight on this tip. **Keep #33 locked.** **No Referee Pass claimed.**
...
21. **Phase 9.1 engine (robust EW / burn-through / HoJ / transponder claim)** — implemented from `docs/phase9/` only on this tip (from `main` @ `4d038e7` after PR #34). Seven gates in engine + S15 probes: spend-to-suppress on reserved `ew` × A×H + dedicated empty-by-default `ew_equipment` slot; burn-through `B × √Q` always positive for a funded receiver; residue-before-void (jam no longer sets `detected: false`); RSS of all paid emitters + true-side labels; HoJ as an unmounted matrix row (`provenance: new`) before any `game_items.json` retune, emission-only / incarnation-lock / silence→coast, bills weapons not `ew`; transponder On/Off/Spoof = claim only, never Phase 1 identity. Magnitudes injectable / TBD. Remastered = shapes only (no `git am`, no locked watts). **Keep #33 locked.** **No Referee Pass claimed.**

## Version history

Newest first.

- **Phase 9.1 engine** — `2026-09-14T12:30:00Z` (Asia/Dubai `2026-09-14T16:30:00+04:00`) — robust EW / burn-through / HoJ / transponder claim from `docs/phase9/` on `cursor/phase91-ew-robustness-engine-8db5` (`main` @ `4d038e7` after PR #34). S15. Magnitudes injectable. No remastered watt lock. No Referee Pass claimed. **#33 stays locked.**
