...
Updated: `2026-09-13T15:40:00Z` (Phase 6.5 brief on `cursor/phase65-sensors-power-brief-1975`)  
Head: docs-only Phase 6.5 proposal from `main` @ `bcb4a9a` (PR #25 content + PR #24 Phase 6 engine)
...
12. **Phase 6.5 brief (power budget / passive vs active draw / upgradeable sensor suites)** — **proposal** from `main` @ `bcb4a9a` (after Phase 6 engine PR #24 + content PR #25). Five hard gates for scoring (authored base generation shared by propulsion/weapons/cloak/sensors/EW; passive quieter-weaker vs active stronger-detectable with different draw; suites as paid equipment including scout-freighter; tank/haul/scout/gun curves with no dominated hull; EW reserved on the same budget, not implemented). **Extends** Phase 6; **does not reopen** gates 1–8 (layers; hidden both sides; same-tick lock drop; variance; useful+detectable scans). Reman 53 and PR #25 aliases unchanged; 172 is the field baseline only (no invented numbers, no catalog wire). Probe sketch S10.x (S9 stays green). **No Referee Pass claimed.** Referee / One score the five gates before any engine PR. Number Three probes only after engine.

## Version history

Newest first.

- **Phase 6.5 brief** — `2026-09-13T15:40:00Z` (Asia/Dubai `2026-09-13T19:40:00+04:00`) — docs-only proposal + engine-deps from `main` @ `bcb4a9a`. Five locks (generation budget; passive/active draw; upgradeable suites; role-curve tradeoffs; EW reserved on same pool). Does not reopen Phase 6 gates 1–8. No Referee Pass claimed.
