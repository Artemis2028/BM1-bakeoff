...
Updated: `2026-09-14T15:10:00Z` (Phase 9.2 EW-depth brief open; Phase 9.1 engine merged)  
Head: Phase 9 engine merged on `main` @ `2b38a14` (PR #33). Phase 9.1 amend brief merged [PR #34](https://github.com/Artemis2028/BM1-bakeoff/pull/34) @ `4d038e7`. Phase 9.1 **engine** merged [PR #35](https://github.com/Artemis2028/BM1-bakeoff/pull/35) @ `d1837fe`. Phase **9.2** depth brief in flight on this tip (lobes, escort ECCM, EW depth finish). **Keep #33 and #35 locked.** **No Referee Pass claimed.**
...
| `docs/phase9/` | Phase 9 EW / separately reviewed weapon behavior proposal + engine deps — brief merged (PR #32); engine implemented from that brief (PR #33, `2b38a14`); EW on P6 layers + P6.5 reserved `ew` budget; ghosts = contact-book rows; jamming cannot unsend delivered P4/P5 reports; weapons matrix before overhaul (Flash table = source, not prices); pursuit ≠ permission ≠ per-weapon gate; boarding out. **#33 stays locked** (do not reopen the six Phase 9 gates). Phase **9.1 amend** brief merged [PR #34](https://github.com/Artemis2028/BM1-bakeoff/pull/34) @ `4d038e7`; engine merged [PR #35](https://github.com/Artemis2028/BM1-bakeoff/pull/35) @ `d1837fe` (spend-to-suppress, burn-through `B × √Q`, residue-before-void, RSS, HoJ matrix row first, transponder claim only). **#35 stays locked** (do not reopen the seven Phase 9.1 gates). Phase **9.2** depth brief open on this tip (directional lobes, shared escort ECCM, spoof catch-path, Phase 7 comms-delay, heat/decoys/silent-running, playtest magnitudes, dock-overlap UI). Magnitudes injectable playtest defaults; remastered EW is background shapes only (no `git am`); **no Referee Pass claimed** |
| `docs/GUIDED-CONVERGENCE.md` | Guided remastered-work product knowledge for remaining packages (weapons table, boarding, standing, catalog wire). Standing tiers + catalog wire are **engine-landed** (PR #28); Flash weapon prices remain **source material, not final prices**. Companion: plan §16 |
...
22. **Phase 9.2 brief (lobes / escort ECCM / EW depth finish)** — **proposal** from `main` @ `d1837fe` (after Phase 9.1 engine PR #35). Eight hard gates for scoring (directional lobes on contest + friendlies in-lobe; escort ECCM detection/last-known/residue only; spoof catch-path no Phase 1 rewrite / no auto-fire; comms-delay new deliver/order only; heat/decoys/silent-running on reserved `ew`; injectable playtest magnitudes; dock-overlap UI polish; preserve #33/#35). Magnitudes injectable / TBD playtest ledger. Remastered EW pack is background shapes only. **Does not** reopen Phase 9 six gates or Phase 9.1 seven gates. Boarding parked. **No Referee Pass claimed.**
