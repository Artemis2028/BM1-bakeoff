...
const output = `${JSON.stringify({
  ...
  ships: source.ships.map((ship) => ({
    ...ship,
    image: String(ship.image || '').startsWith('bm-ships/') || String(ship.image || '').startsWith('assets/game/')
      ? ship.image
      : `bm-ships/${String(ship.image || '').replace(/^\.\//, '')}`,
  })),
}, null, 2)}\n`;
const target = new URL('../data/starship_manifest.json', import.meta.url);
if (process.argv.includes('--check')) {
  if (fs.readFileSync(target, 'utf8') !== output) {
    throw new Error('Generated ship manifest is stale. Run node scripts/sync-ship-roster.mjs.');
  }
